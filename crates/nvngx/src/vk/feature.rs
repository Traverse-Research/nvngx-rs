...
impl FeatureParameters {
    ...
    pub fn new_vk() -> Result<Self> {
        let mut ptr: *mut nvngx_sys::NVSDK_NGX_Parameter = std::ptr::null_mut();
        Result::from(unsafe {
            nvngx_sys::vk::NVSDK_NGX_VULKAN_AllocateParameters(&mut ptr as *mut _)
        })
        .map(|()| unsafe { Self::from_raw(ptr, nvngx_sys::vk::NVSDK_NGX_VULKAN_DestroyParameters) })
    }
}
...
impl Feature {
    ...
    pub fn new_vk(
        ...
        let mut handle = FeatureHandle::new(nvngx_sys::vk::NVSDK_NGX_VULKAN_ReleaseFeature);
        Result::from(unsafe {
            nvngx_sys::vk::NVSDK_NGX_VULKAN_CreateFeature1(
                device,
                command_buffer,
                feature_type,
                parameters.ptr,
    ...
    pub fn get_scratch_buffer_size_vk(&self) -> Result<usize> {
        let mut size = 0usize;
        Result::from(unsafe {
            nvngx_sys::vk::NVSDK_NGX_VULKAN_GetScratchBufferSize(
                self.feature_type,
    ...
    pub fn evaluate_vk(&self, command_buffer: vk::CommandBuffer) -> Result {
        unsafe {
            nvngx_sys::vk::NVSDK_NGX_VULKAN_EvaluateFeature_C(
                command_buffer,
                self.handle.ptr,
                self.parameters.ptr,
