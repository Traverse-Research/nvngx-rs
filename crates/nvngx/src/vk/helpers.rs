...
//!
//! This is a mechanical port of [`nvngx_sys::helpers`] that routes every FFI
//! call through a [`Dispatch`] reference instead of calling free functions
//! directly, so the same code works for both the `linked` and `loaded`
//! features.

use ash::vk::Handle;

use nvngx_sys::{NVSDK_NGX_GBufferType::*, *};
...
pub(crate) unsafe fn dlss_get_optimal_settings(
    dispatch: &Dispatch,
    ...
    dispatch.NVSDK_NGX_Parameter_GetVoidPointer(
        p_in_params,
    ...
    dispatch.NVSDK_NGX_Parameter_SetUI(
        p_in_params,
    ...
    dispatch.NVSDK_NGX_Parameter_SetI(
        p_in_params,
    ...
    dispatch.NVSDK_NGX_Parameter_SetI(p_in_params, NVSDK_NGX_Parameter_RTXValue.as_ptr().cast(), 0);
    ...
    dispatch.NVSDK_NGX_Parameter_GetUI(
        p_in_params,
    ...
    *p_out_render_min_height = *p_out_render_optimal_height;
    ...
    dispatch.NVSDK_NGX_Parameter_GetF(
        p_in_params,
...
#[allow(clippy::too_many_arguments)]
pub(crate) unsafe fn vulkan_create_dlss_ext1(
    dispatch: &Dispatch,
    ...
    if !in_device.is_null() {
        ...
        dispatch.NVSDK_NGX_VULKAN_CreateFeature(
            in_cmd_list,
            NVSDK_NGX_Feature::NVSDK_NGX_Feature_SuperSampling,
            p_in_params,
            pp_out_handle,
...
///
/// All pointer parameters must be valid.
pub(crate) unsafe fn vulkan_evaluate_dlss_ext(
    dispatch: &Dispatch,
    in_cmd_list: ash::vk::CommandBuffer,
    ...
    macro_rules! set_ptr {
        ($name:ident, $val:expr) => {
            dispatch.NVSDK_NGX_Parameter_SetVoidPointer(
                p_in_params,
                $name.as_ptr().cast(),
                $val as *mut _,
            );
    ...
    macro_rules! set_f {
        ($name:ident, $val:expr) => {
            dispatch.NVSDK_NGX_Parameter_SetF(p_in_params, $name.as_ptr().cast(), $val);
        };
    }
    macro_rules! set_ui {
        ($name:ident, $val:expr) => {
            dispatch.NVSDK_NGX_Parameter_SetUI(p_in_params, $name.as_ptr().cast(), $val);
        };
    }
    macro_rules! set_i {
        ($name:ident, $val:expr) => {
            dispatch.NVSDK_NGX_Parameter_SetI(p_in_params, $name.as_ptr().cast(), $val);
        };
    }
    ...
    dispatch.NVSDK_NGX_VULKAN_EvaluateFeature_C(in_cmd_list, p_in_handle, p_in_params, None)
}
...
/// All pointer parameters must be valid.
#[allow(clippy::too_many_arguments)]
pub(crate) unsafe fn vulkan_create_dlssd_ext1(
    dispatch: &Dispatch,
    in_device: ash::vk::Device,
    in_cmd_list: ash::vk::CommandBuffer,
    in_creation_node_mask: u32,
...
///
/// All pointer parameters must be valid.
pub(crate) unsafe fn vulkan_evaluate_dlssd_ext(
    dispatch: &Dispatch,
    in_cmd_list: ash::vk::CommandBuffer,
    p_in_handle: *mut NVSDK_NGX_Handle,
    p_in_params: *mut NVSDK_NGX_Parameter,
